...
const adminSidebarItems: SidebarItem[] = [
  { title: "Dashboard", translationKey: "dashboard", icon: Home, path: "/syndic-demo/dashboard", description: "Overview, unpaid charges, tickets, announcements" },
  { title: "Buildings", translationKey: "buildings", icon: Building2, path: "/syndic-demo/buildings", description: "Buildings, floors, and apartments" },
  { title: "Users", translationKey: "users", icon: Users, path: "/syndic-demo/users", description: "Manage Owner and Tenant accounts" },
  { title: "Charges", translationKey: "charges", icon: CreditCard, path: "/syndic-demo/charges", description: "Generate and track monthly charges" },
  { title: "Helpdesk", translationKey: "helpDesk", icon: TicketCheck, path: "/syndic-demo/helpdesk", description: "View and manage all tickets" },
  { title: "Documents", translationKey: "documents", icon: FolderOpen, path: "/syndic-demo/documents", description: "Upload and organize files" },
  { title: "Announcements", translationKey: "announcements", icon: Megaphone, path: "/syndic-demo/announcements", description: "Post notices for all users" },
]

// Owner pages
const ownerSidebarItems: SidebarItem[] = [
  { title: "Dashboard", translationKey: "dashboard", icon: Home, path: "/syndic-demo/dashboard", description: "Balance and announcements" },
  { title: "My Charges", translationKey: "myCharges", icon: CreditCard, path: "/syndic-demo/my-charges", description: "Charge history and receipts" },
  { title: "Documents", translationKey: "documents", icon: FolderOpen, path: "/syndic-demo/documents", description: "Download shared files" },
  { title: "My Tickets", translationKey: "myTickets", icon: TicketCheck, path: "/syndic-demo/my-tickets", description: "Submit and track incidents" },
]

// Tenant pages
const tenantSidebarItems: SidebarItem[] = [
  { title: "Dashboard", translationKey: "dashboard", icon: Home, path: "/syndic-demo/dashboard", description: "Announcements" },
  { title: "My Tickets", translationKey: "myTickets", icon: TicketCheck, path: "/syndic-demo/my-tickets", description: "Submit and track complaints" },
]
...
const allowedRoutes: Record<UserRole, string[]> = {
  Admin: ["/syndic-demo/dashboard", "/syndic-demo/users", "/syndic-demo/buildings", "/syndic-demo/charges", "/syndic-demo/helpdesk", "/syndic-demo/documents", "/syndic-demo/announcements", "/syndic-demo/profile"],
  Owner: ["/syndic-demo/dashboard", "/syndic-demo/my-charges", "/syndic-demo/documents", "/syndic-demo/my-tickets", "/syndic-demo/profile"],
  Tenant: ["/syndic-demo/dashboard", "/syndic-demo/my-tickets", "/syndic-demo/profile"],
}
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent side="top" align="start" className="w-[12.5rem] mb-2 bg-white border-none shadow-lg rounded-sm p-1.5">
          <DropdownMenuItem asChild className="cursor-pointer hover:bg-black/5 focus:bg-black/5 focus:text-black rounded-sm py-2.5 transition-colors group">
            <Link href="/syndic-demo/profile" className="flex items-center gap-2.5 w-full">
...
function SyndicLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  const isAuthPage = pathname === "/syndic-demo/login" || pathname === "/syndic-demo/register"
  const isDashboardPath = pathname?.startsWith("/syndic-demo") && !isAuthPage && pathname !== "/syndic-demo"
  ...
  useEffect(() => {
    if (userData) {
      ...
      } else if (pathname === "/syndic-demo/profile") {
